...
final class HeartbeatTest extends AbstractProtocolTest
{
    ...
    public function testSend()
    {
        ...
        $data = $protocol->pack();
        SocketServer::getInstance(false, false)->run($coordinatorResponse->getHost()->getValue(),
            $coordinatorResponse->getPort()->getValue(), function () use ($data) {
                return $data;
            }, function (string $data, Client $client) use ($protocol) {
                $protocol->response->unpack($data, $client);
        ...
        $protocol->setGenerationId(Int32::value($joinGroupResponse->getGenerationId()->getValue()))
                 ->setMemberId(String16::value($joinGroupResponse->getMemberId()->getValue()))
        ...
        $heartbeatResponse = $protocol->response;
        $this->assertEquals(ProtocolErrorEnum::NO_ERROR, $heartbeatResponse->getErrorCode()->getValue());
    }
}
